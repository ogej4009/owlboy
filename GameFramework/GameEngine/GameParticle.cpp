...
{
	{
		...
		m_pParticleBuffer->Create(sizeof(Particle), ParticleCount, &PData[0], D3D11_USAGE_DYNAMIC);
	}

	{
		SData.resize(ParticleCount);
		...
		m_pSharedBuffer->Create(sizeof(ParticleShared), ParticleCount, &SData[0]);
	}
	...
	m_RP = m_Render->CreateRenderPlayer(m_Mesh, "Particle");

	m_RP->SetTexture(L"Tex", L"T_fire_trail_bluredori.tga");

	m_Mesh->InstancedRenderCount(ParticleCount);

	m_RP->m_IsDebug = true;
	m_RP->SetStructuredBuffer("arrParticle", m_pParticleBuffer);
	m_RP->SetStructuredBuffer("ParticleShared", m_pSharedBuffer);
}
...
{
	{
		...
		m_pParticleBuffer->SetData(&PData[0], sizeof(Particle) * ParticleCount);
	}
}
