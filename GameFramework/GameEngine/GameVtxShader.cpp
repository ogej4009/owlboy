...
unsigned int GameVtxShader::FmtSize(DXGI_FORMAT _Fmt)
{
	switch (_Fmt)
	{
	case DXGI_FORMAT_UNKNOWN:
		MSG_ASSERT_CODE(L"포맷을 알수가 없습니다");
...
void GameVtxShader::Load(const GameString& _FuncName, unsigned int _VH, unsigned int _VL)
{
	m_VH = _VH;
	m_VL = _VL;
	...
	if (S_OK != D3DCompileFromFile(
		...
		FuncName.c_str(),					// 컴파일하고 싶은 함수 이름
		Arr,								// 버전
		D3DCOMPILE_PACK_MATRIX_ROW_MAJOR,	//  column->row 행렬을 읽는 방식을 바꿔서 전달한다. 전치한다. transposed 그렇게 해석하겠다. 
		0,									// 기본 옵션으로 컴파일
		&m_pBlob,
		&m_pErrBlob)
		)
	{
		...
		MSG_ASSERT_CODE(ErrorText + L"쉐이더 컴파일에러");
		return;
	}

	if (S_OK != GameDevice::MainObj()->Device()->CreateVertexShader(
...
{
	GameDevice::MainObj()->Context()->IASetInputLayout(m_pLayOut);
	GameDevice::MainObj()->Context()->VSSetShader(m_ShaderPtr, 0, 0);
}

void GameVtxShader::AddLayout(
...
void GameVtxShader::CreateLayout()
{
	if (S_OK != GameDevice::MainObj()->Device()->CreateInputLayout(
		&m_ArrInputList[0],
		...
		&m_pLayOut))
	{
		MSG_ASSERT_CODE(L"인풋 레이아웃 생성에 실패했습니다.");
	}
