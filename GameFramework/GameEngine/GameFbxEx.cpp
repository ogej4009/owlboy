...
//#include <WGAMEFILE.h>
//#include "WVBBUFFER.h"
//#include "WIBBUFFER.h"// #include "JK3DEngineDebug.h"
//#include "WMESH.h"
//#include <algorithm>
//#include "WTEXTURE.h"
//
...
//}
//
//void WFbxEx::Load(float4 modifyRotationAxis)
//{
...
//
//			m_SceneInfo.MeshInfo.push_back(WFbxExMeshInfo());
//			WFbxExMeshInfo& meshInfo = m_SceneInfo.MeshInfo.at(m_SceneInfo.MeshInfo.size() - 1);
...
//	{
//		WFbxExMeshInfo& meshInfo = m_SceneInfo.MeshInfo.at(n);
...
//		{
//			pDrawSet->m_MapWI.insert(std::make_pair(pMesh, std::map<int, std::vector<WFbxExIW>>()));
//		}
//
...
//
//					//Uv를 구할 때 그냥 이렇게 하면된다 mesh를 통해서 구하는 법
//					fbxsdk::FbxVector2 Uv = meshInfo.pMesh->GetElementUV(0)->GetDirectArray().GetAt(ControlPointIndex);
//
//					VtxData[ControlPointIndex].Uv.x = (float)Uv.mData[0];
//					VtxData[ControlPointIndex].Uv.y = 1.0f - (float)Uv.mData[1];
//				}
//			}
//		}
//
...
//}
//
//WVECTOR WFbxEx::ReadUv(FbxMesh* mesh, int ControlPointIndex, int vertexCounter)
//{
//	FbxGeometryElementUV* vertexUv = mesh->GetElementUV(0);
//
//	FbxVector2 Uv = vertexUv->GetDirectArray().GetAt(ControlPointIndex);
//
//	WVECTOR result;
//
//	result.x = (float)Uv.mData[0];
//	result.y = (float)Uv.mData[1];
//
//	return result;
//}
//
//void WFbxEx::ImportBone()
//{
//	size_t meshCount = m_SceneInfo.MeshInfo.size();
//	if (0 == meshCount)
//	{
...
//
//			WGAMESTRING tempLinkName = Link->GetName();
//			WGAMESTRING tempAltLinkName = AltLink->GetName();
...
//	WGAMESTRING LinksWithoutBindPoses;
//	int NumberOfRoot = 0;
//
//	int RootIdx = -1;
//
...
//}
//
//void WFbxEx::LoadSkinAndCluster()
//{
//	std::map<int, DRAWSET*>::iterator iter = m_mapDrawsets.begin();
//	std::map<int, DRAWSET*>::iterator iterEnd = m_mapDrawsets.end();
//	for (; iter != iterEnd; ++iter)
...
//}
//
//void WFbxEx::DrawSetMeshesCreate()
//{
//	std::map<int, DRAWSET*>::iterator iter = m_mapDrawsets.begin();
//	std::map<int, DRAWSET*>::iterator iterEnd = m_mapDrawsets.end();
//	for (; iter != iterEnd; ++iter)
...
//}
//
//fbxsdk::FbxNode* WFbxEx::RecursiveFindParentLodGroup(fbxsdk::FbxNode* parentNode)
//{
//	if (nullptr == parentNode)
//	{
//		return nullptr;
...
//}
//
//fbxsdk::FbxNode* WFbxEx::FindLODGroupNode(fbxsdk::FbxNode* NodeLodGroup, int LodIndex, fbxsdk::FbxNode* NodeToFind)
//{
...
//}
//
//fbxsdk::FbxNode* WFbxEx::RecursiveGetFirstMeshNode(fbxsdk::FbxNode* Node, fbxsdk::FbxNode* NodeToFind)
//{
//	if (Node == nullptr)
//	{
//		return nullptr;
...
//}
//
//void WFbxEx::TraverseHierarchyNodeRecursively(WFbxExSceneInfo& SceneInfo, fbxsdk::FbxNode* ParentNode, WFbxExNodeInfo& ParentInfo)
//{
//	int NodeCount = ParentNode->GetChildCount();
//	for (int NodeIndex = 0; NodeIndex < NodeCount; ++NodeIndex)
//	{
//		fbxsdk::FbxNode* ChildNode = ParentNode->GetChild(NodeIndex);
//		WFbxExNodeInfo ChildInfo;
...
//}
//
//fbxsdk::FbxAMatrix WFbxEx::ComputeTotalMatrix(fbxsdk::FbxNode* Node)
//{
...
//}
//
//bool WFbxEx::IsOddNegativeScale(const fbxsdk::FbxAMatrix& TotalMatrix)
//{
...
//}
//
//void WFbxEx::DrawSetMaterialSetting(fbxsdk::FbxNode* _Node, DRAWSET* _DrawData)
//{
//	int MtrlCount = _Node->GetMaterialCount();
//
//	if (MtrlCount > 0)
//	{
//		_DrawData->m_MatialData.push_back(std::vector<WFbxExMaterialData>());
//
//		std::vector<WFbxExMaterialData>& MatrialSet = _DrawData->m_MatialData[_DrawData->m_MatialData.size() - 1];
//
...
//			}
//
//			MatrialSet.push_back(WFbxExMaterialData());
//			WFbxExMaterialData& MatData = MatrialSet[MatrialSet.size() - 1];
...
//			{
//				if (true == WGAMEPATH::IsExits(MatData.DifTexture))
//				{
//					WTEXTURE::Load(MatData.DifTexture);
//					MatData.DifTexture = WGAMEIO::FileName(MatData.DifTexture);
//				}
//
//			}
//
...
//			{
//				if (true == WGAMEPATH::IsExits(MatData.BmpTexture))
//				{
//					WTEXTURE::Load(MatData.BmpTexture);
//					MatData.BmpTexture = WGAMEIO::FileName(MatData.BmpTexture);
//				}
//
//			}
//
...
//			{
//				if (true == WGAMEPATH::IsExits(MatData.SpcTexture))
//				{
//					WTEXTURE::Load(MatData.SpcTexture);
//					MatData.SpcTexture = WGAMEIO::FileName(MatData.SpcTexture);
//				}
//			}
//		}
//
...
//}
//
//float4 WFbxEx::MaterialColor(fbxsdk::FbxSurfaceMaterial* pMtrl, const char* _ColorName, const char* _FactorName)
//{
...
//}
//
//float WFbxEx::MaterialFactor(fbxsdk::FbxSurfaceMaterial* pMtrl, const char* _FactorName) {
//	double dFactor = 0;
//	FbxProperty FactorPro = pMtrl->FindProperty(_FactorName);
//
...
//}
//
//WGAMESTRING WFbxEx::MaterialTex(fbxsdk::FbxSurfaceMaterial* pMtrl, const char* _FactorName)
//{
...
//}
//
//void WFbxEx::LoadUv(fbxsdk::FbxMesh* _Mesh, std::vector<VTX3DMESHIN>& _ArrVtx, int VtxId, int _Index)
//{
//	std::map<WGAMESTRING, int> UVSets;
...
//				{
//					WGAMESTRING uvName = elementUv->GetName();
//					if (true == uvName.IsEmpty())
//					{
//						uvName.Appendf("UVmap_%d", uvLayerIndex);
//					}
//					std::map<WGAMESTRING, int>::iterator findIter = UVSets.find(uvName);
...
//	_ArrVtx[_Index].Uv.y = (float)(1.0f - (vUV.mData[1]));
//
//}
//
//void WFbxEx::LoadBinormal(fbxsdk::FbxMesh* _Mesh, fbxsdk::FbxAMatrix _MeshMatrix, std::vector<VTX3DMESHIN>& _ArrVtx, int VtxId, int _Index)
//{
...
//}
//
//void WFbxEx::LoadTangent(fbxsdk::FbxMesh* _Mesh, fbxsdk::FbxAMatrix _MeshMatrix, std::vector<VTX3DMESHIN>& _ArrVtx, int VtxId, int _Index)
//{
...
//}
//
//void WFbxEx::LoadNormal(fbxsdk::FbxMesh* _Mesh, fbxsdk::FbxAMatrix _MeshMatrix, std::vector<VTX3DMESHIN>& _ArrVtx, int VtxId, int _Index)
//{
...
//}
//
//void WFbxEx::DrawSetMeshCreate(DRAWSET* _DrawData)
//{
...
//			WPTR<WIBBUFFER> IB = new WIBBUFFER();
...
//}
//
//void WFbxEx::LoadAnimationVertexData(DRAWSET* _DrawData, const std::vector<ClusterData>& vecClusterData)
//{
...
//		}
//
//		WGAMESTRING StrBoneName = clusterData.LinkName;
...
//}
//
//void WFbxEx::CalAnimationVertexData(DRAWSET* _DrawData)
//{
...
//					[](const WFbxExIW& _Left, const WFbxExIW& _Right) {
...
//
//				std::vector<WFbxExIW>::iterator IterErase = _WI.second.begin() + 4;
...
//}
//
//void WFbxEx::DrawSetWeightAndIndexSetting(DRAWSET* _DrawSet, fbxsdk::FbxMesh* _Mesh, fbxsdk::FbxCluster* _Cluster, int _BoneIndex)
//{
...
//}
//
//Bone* WFbxEx::FindBone(const WGAMESTRING& _boneName)
//{
...
//}
//
//Bone* WFbxEx::FindBone(const int Index)
//{
...
//
//
//int WFbxEx::GetFbxMeshCount()
//{
//	return (int)m_SceneInfo.MeshInfo.size();
//}
//
//WGAMESTRING WFbxEx::GetMeshNameByIndex(const int Index)
//{
...
//}
//
//bool WFbxEx::RetrievePoseFromBindPose(fbxsdk::FbxScene* pScene, const std::vector<fbxsdk::FbxNode*>& NodeArray, fbxsdk::FbxArray<fbxsdk::FbxPose*>& PoseArray)
//{
...
//		if (CurrentPose && CurrentPose->IsBindPose())
//		{
//			WGAMESTRING PoseName = CurrentPose->GetName();
//			fbxsdk::FbxStatus Status;
//
//			for (auto Current : NodeArray)
//			{
//				WGAMESTRING CurrentName = Current->GetName();
...
//						else
//						{
//							WGAMESTRING ErrorString = Status.GetErrorString();
//							WGAMESTRING CurrentName = Current->GetName();
...
//}
//
//void WFbxEx::BuildSkeletonSystem(fbxsdk::FbxScene* pScene, std::vector<fbxsdk::FbxCluster*>& ClusterArray, std::vector<fbxsdk::FbxNode*>& OutSortedLinks)
//{
//	fbxsdk::FbxNode* Link;
...
//}
//
//fbxsdk::FbxNode* WFbxEx::GetRootSkeleton(fbxsdk::FbxScene* pScene, fbxsdk::FbxNode* Link)
//{
...
//}
//
//void WFbxEx::RecursiveBuildSkeleton(fbxsdk::FbxNode* Link, std::vector<fbxsdk::FbxNode*>& OutSortedLinks)
//{
//	if (IsBone(Link))
//	{
...
//}
//
//bool WFbxEx::IsBone(fbxsdk::FbxNode* Link)
//{
...
//}
//
//bool WFbxEx::IsNull(fbxsdk::FbxNode* Link)
//{
//	fbxsdk::FbxNodeAttribute* Attr = Link->GetNodeAttribute();
//	if (Attr)
//	{
...
//}
//
//int WFbxEx::GetDrawSetCount()
//{
//	return (int)m_mapDrawsets.size();
//}
//
//DRAWSET* WFbxEx::GetDrawSet(int LodLevel)
//{
...
//
//
//WMATRIX WFbxEx::FbxMatToWMATRIX(const fbxsdk::FbxAMatrix& _BaseTrans)
//{
//	WMATRIX Mat;
//
//	for (int y = 0; y < 4; y++)
...
//}
//
//fbxsdk::FbxAMatrix WMATRIXToFbxAMatrix(const WMATRIX& _WMATRIX)
//{
...
//		for (int x = 0; x < 4; x++)
//		{
//			mat.mData[y].mData[x] = _WMATRIX.Arr2D[y][x];
//		}
//	}
//
//	return mat;
//}
//
//float4 WFbxEx::FbxVecTofloat4(const fbxsdk::FbxVector4& _BaseVector)
//{
//	float4 Vec;
...
//}
//
//float4 WFbxEx::FbxVecToTransform(const fbxsdk::FbxVector4& _BaseVector)
//{
//	float4 Vec;
//	Vec.Arr[0] = (float)_BaseVector.mData[0];
//	Vec.Arr[1] = (float)_BaseVector.mData[1];
...
//}
//
//float4 WFbxEx::FbxQuaternionTofloat4(const fbxsdk::FbxQuaternion& _BaseQ)
//{
//	float4 Vec;
...
//	return true;
//}
//bool WFbxEx::IsSkeleton()
//{
//	return true;
//}
//bool WFbxEx::IsAnimation()
//{
//	return 0 != m_UserAniData.size();
//}
//
//void WFbxEx::CalWFbxExBoneFrameTransMatrix(WPTR<WFbxEx> _Fbx)
//{
...
//	}
//
//	ProcessAnimationLoad(_Fbx, _Fbx->m_pFbxRootNode);
//
//	ProcessAnimationCheckState(_Fbx);
//}
//
//void WFbxEx::ProcessAnimationLoad(WPTR<WFbxEx> _Fbx, fbxsdk::FbxNode* pNode)
//{
...
//			{
//				ProcessAnimationLoad(_Fbx, pNode->GetChild(0));
//				return;
//			}
//		}
//		break;
...
//	{
//		ProcessAnimationLoad(_Fbx, pNode->GetChild(n));
//	}
//}
//
//void WFbxEx::ProcessAnimationCheckState(WPTR<WFbxEx> _Fbx)
//{
//	size_t userAniDataSize = m_UserAniData.size();
//	for (size_t userAniDataIndex = 0; userAniDataIndex < userAniDataSize; ++userAniDataIndex)
//	{
//		WFbxExAniData& userAniData = m_UserAniData.at(userAniDataIndex);
...
//		{
//			WFbxExBoneFrame& aniFrameData = userAniData.m_AniFrameData.at(aniFrameDataIndex);
...
//				{
//					WFbxExBoneFrame& parentAniFrameData = userAniData.m_AniFrameData.at(curBone.ParentIndex);
//					for (fbxsdk::FbxLongLong start = 0; start < fbxTime; ++start)
//					{
//						aniFrameData.m_Data[start].Time = parentAniFrameData.m_Data[start].Time;
//						aniFrameData.m_Data[start].LocalAnimation = WMATRIXToFbxAMatrix(curBone.BonePos.Local);
...
//						aniFrameData.m_Data[start].FrameMat = FbxMatToWMATRIX(aniFrameData.m_Data[start].GlobalAnimation);
...
//}
//
//void WFbxEx::MeshAnimationLoad(WPTR<WFbxEx> _Fbx, fbxsdk::FbxNode* pNode)
//{
//	std::vector<WFbxExBoneFrame> vecAniFrameData;
...
//				WGAMESTRING currJointName = pCurrCluster->GetLink()->GetName();
...
//
//					WGAMESTRING linkName = pCurrCluster->GetLink()->GetName();
...
//					m_UserAniData[userAniDataIndex].m_AniFrameData[pBone->Index].m_Data[fixIndex].FrameMat = FbxMatToWMATRIX(m_UserAniData[userAniDataIndex].m_AniFrameData[pBone->Index].m_Data[fixIndex].GlobalAnimation);
...
//}
//
//fbxsdk::FbxAMatrix WFbxEx::GetGeometryTransformation(fbxsdk::FbxNode* pNode)
//{
