...
#pragma region  GRID MESH
	/* GRID MESH입니다. */
	{
		std::vector<Vtx2D> m_GRID;
		m_GRID.resize(4);
		/* 기본 매쉬를 만들때는 크기를 1로 만들어 놓는 경우가 대부분입니다.		*/
		m_GRID[0].Pos = { -0.5f, 0.0f, 0.5f, 1.0f };
		m_GRID[1].Pos = { 0.5f, 0.0f, 0.5f, 1.0f };
		m_GRID[2].Pos = { 0.5f, 0.0f, -0.5f, 1.0f };
		m_GRID[3].Pos = { -0.5f, 0.0f, -0.5f, 1.0f };
		m_GRID[0].Color = m_GRID[1].Color = m_GRID[2].Color = m_GRID[3].Color = CVector::WHITE;
		GameVtxBuffer::Create<Vtx2D>(L"GridMesh", m_GRID);
		std::vector<UINT> m_IDX = { 0, 1, 2, 0, 2, 3 };
		GameIdxBuffer::Create<UINT>(L"GridMesh", m_IDX);
		GameMesh::Create(L"GridMesh");
	}
#pragma endregion
