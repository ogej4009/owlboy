...
void GameTransform::TransformUpdate()
{
	...
	if (true == CalMatrixCheck[(int)MRX_BASE::MRX_SCALE])
	{
		...
		CalMatrixCheck[(int)MRX_BASE::MRX_LWORLD] = true;
	}

	if (true == CalMatrixCheck[(int)MRX_BASE::MRX_ROT])
	{
		m_DefTD.ROT.RotDeg(m_LRot);
		CalMatrixCheck[(int)MRX_BASE::MRX_LWORLD] = true;
	}

	if (true == CalMatrixCheck[(int)MRX_BASE::MRX_POS])
	{
		m_DefTD.POS.Pos3d(m_LPos);
		CalMatrixCheck[(int)MRX_BASE::MRX_LWORLD] = true;
	}
	...
	//	○ 디폴트
		...
		CalMatrixCheck[(int)MRX_BASE::MRX_WWORLD] = true;
...
{
	CMatrix PV = m_DefTD.PROJ * m_DefTD.VIEW;
	Pos = Pos * PV;
