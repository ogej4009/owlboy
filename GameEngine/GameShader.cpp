...
void GameShader::AutoShaderCompile(const GameString& FileName)
{
	// [ ] 
	////////////////////////////////////////// 테스트 
	if (std::wstring::npos != FileName.Find("Particle.hlsl")) 
	{
		int a = 0;
	}
	
	if (std::wstring::npos != FileName.Find("OutLineDraw.hlsl"))
	{
		int a = 0;
	}

	if (std::wstring::npos != FileName.Find("MotionBlurDraw.hlsl"))
	{
		int a = 0;
	}
...

	{
		...
		{
			////////////////////////// 테스트 
			if (std::wstring::npos != NameStart->StrText.Find(L"GS_Particle")) /// GS
			{
				int a = 0;
			}
			...
			else if (L"point" == (*ParaStart).StrText) /// GS
			{
				++NameStart;
				++ParaStart;
				// 지오메트리 쉐이더를 고려
...

	{
		...
		for (; NameStart != NameEnd; ++NameStart)
		{
			...
			if (L"void" == ReturnData.StrText)  //// GS
			{
